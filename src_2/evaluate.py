...
def evaluate_recommender(model, train, test):
    ...
    test_pd['actualrating'] = test_pd['rating']
    test_pd.drop('rating', axis=1, inplace=True)
    predicted_pd = predicted.toPandas()
    if 'prediction' in predicted_pd.columns.values:
        predicted_pd['rating'] = predicted_pd['prediction']
    return compute_score(predicted_pd[['user', 'movie', 'rating']],
                         test_pd[['user', 'movie', 'actualrating']])
